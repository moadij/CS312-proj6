...
# The class I wrote to represent a state. It contains the state's
# lowerbound, cost matrix, partial path, and depth.
class State:
    def __init__(self, _id, bound, matrix, partialPath, depth):
        self._id = _id
        self._bound = bound
        self._matrix = matrix
    ...
    # The less that function I overrode for priority queue insertion
    # First, I check if the partial paths are the same length. If they are,
    # I check the bound. If they aren't, I still check the bound, but am
    # more lenient with lower depths.
    # Time Complexity: O(1)
    # Space Complexity: O(1)
    def __lt__(self, other):
        if len(self._partialPath) is len(other._partialPath):
            return self._bound < other._bound
        if len(self._partialPath) < len(other._partialPath):
            if self._bound < other._bound:
                return True
            else:
                return (self._bound - other._bound) < (other._bound / 2)
        return False
    
    # To string I overrode for debugging. Not currently used in my code.
    def __str__(self):
        return ''.join((str(self._id), ' ', str(self._bound)))